...
def sPredict(sFactor,sLabel,test_list,train_texts_orig,flag):
    ...
    for text in test_list:
        ...
        #计算分割权重
        if sLabel==0:
            ws1.write(row, (sFactor+1), str(coef))
        elif sLabel==1:
            ws2.write(row, (sFactor+1), str(coef))
        elif sLabel==2:
            ws3.write(row, (sFactor+1), str(coef))
        
# 打开验证集
workbook2 = xlrd.open_workbook(r'validationSet.xls')#赛题中选用sentiment_analysis_validationset
worksheet2 = workbook2.sheet_by_name(u'Sheet1')
#print(worksheet2.cell_value(1,1))
# 将操作文件对象拷贝，变成可写的workbook对象
new_excel = copy(workbook2)
# ws1，ws2，ws3用来训练计算分割权重
ws1 = new_excel.get_sheet(1)
ws2 = new_excel.get_sheet(2)
ws3 = new_excel.get_sheet(3)
#print(ws)
